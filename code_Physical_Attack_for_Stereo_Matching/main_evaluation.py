...
parser.add_argument('--kitti-data', dest='kitti_data', default="",
                    help='path to kitti dataset')
parser.add_argument('--sceneflow-data', dest='sceneflow_data', default="",
                    help='path to sceneflow dataset')
parser.add_argument('--patch-path', dest='patch_path', default='checkpoints/demo/PSMNet/checker_init_pgd_cCV-lr-1e3-mc5-p0_3/patch_epoch_5',
...
def validate_stereo_with_gt(patch, mask, patch_shape, val_loader, stereo_net,epoch, logger, output_writer,errors,error_names,dirs):
    ...
    with torch.no_grad():
        for i, ( tgt_img, ref_img_right, disp_gt,_, fn) in enumerate(val_loader):
            ...
            epe = compute_epe(gt=disp_gt_var, pred=stereo_fwd,mask=mask3pixel) 
